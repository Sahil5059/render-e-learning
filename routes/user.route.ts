...
//STEP: 18
userRouter.post('/activate-user', activateUser); //don't forget to import "activateUser"
//watch: 2:40:21 to 2:46:16
//now, will set up "user login-logout". Watch: 2:46:17 to 2:47:45
//OVER: 18 ("m": ../models.user.model.ts)

//STEP: 25
userRouter.post('/login', loginUser); //don't forget to import "loginUser". Also, watch: 3:08:08 to 3:09:31 
...
//STEP: 28
userRouter.get('/logout',isAuthenticated, logoutUser); //don't forget to import "isAuthenticated". Also, watch- 3:18:35 to 3:21:36
//OVER: 28 ("m": ../controllers/user.controller.ts)

//STEP: 32
userRouter.get("/refresh", updateAccessToken); //don't forget to import "updateAccessToken". Also, watch- 3:35:35 to 3:38:15
...
//STEP: 35
userRouter.get("/me", isAuthenticated, getUserInfo); //don't forget to import "getUserInfo". Also, watch- 3:41:30 to 3:43:00
//OVER: 35

//STEP: 37
userRouter.post("/social-auth", socialAuth); //don't forget to import "socialAuth". Also, watch- 3:45:21 to 3:49:05
//now, we will set up the code for updating user info, password and avatar
//OVER: 37("m": ../controllers/user.controller.ts)

//STEP: 39
userRouter.put("/update-user-info", isAuthenticated, updateUserInfo); //don't forget to import "updateUserInfo". Also, watch- 3:56:50 to 3:59:40
//OVER: 39("m": ../controllers/user.controller.ts)

//STEP: 41
userRouter.put("/update-user-password", isAuthenticated, updatePassword); //don't forget to import "updatePassword". Also, watch- 4:03:50 - 4:07:05
//open the "server" folder terminal and type: "npm i cloudinary"
//OVER: 41("m": ../controllers/user.controller.ts)

//STEP: 46
userRouter.put("/update-user-avatar", isAuthenticated, updateProfilePicture); //don't forget to import "updateProfilePicture". Also, watch- 4:18:10 to
...
//STEP: 94
userRouter.get("/get-users", isAuthenticated, authorizeRoles("admin"), getAllUsers); //don't forget to import "athorizeRoles()" & "getAllUsers". Also, watch- 7:20:50 to 7:23:45
//now, we will set up code to "add memebers" and "get members" in admin dashboard 
//OVER: 94("m": ../services/user.service.ts)

//STEP: 97
userRouter.put("/update-user", isAuthenticated, authorizeRoles("admin"), updateUserRole); //don't forget to import "updateUserRole". Also, watch- 7:27:00 to 7:28:45.
//now, we will set up course for deleting user
//OVER: 97("m": ../controllers/user.controller.ts)

//STEP: 99
userRouter.delete("/delete-user/:id", isAuthenticated, authorizeRoles("admin"), deleteUser); //don't forget to import "deleteUser". Also, watch- 7:31:25 to 7:33:15
